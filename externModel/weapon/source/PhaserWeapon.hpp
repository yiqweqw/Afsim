...
#include "WsfUtil.hpp"
// WsfImplicitWeapon include removed â€?base class is now WsfRF_Jammer (see WsfRF_Jammer.hpp)
...
class JAMMER_CSSC_01_EXPORT PhaserWeapon : public WsfRF_Jammer
{
public:
	class JammerBeam;
	class JammerMode;
	...
	PhaserWeapon(const WsfScenario& aScenario);

	PhaserWeapon(const PhaserWeapon& aSrc);

	PhaserWeapon& operator=(const PhaserWeapon&) = delete;

	~PhaserWeapon() override = default;
	...
	//! Walk every RF receiver in the simulation and add each of our jammer
	//! transmitters to its interference-interactor list (if frequencies overlap).
	//! Tracks every (rcvr,xmtr) pair so DeregisterInterferenceInteractors can
	//! undo it exactly. Called from TurnOn().
	void RegisterInterferenceInteractors();

	//! Remove every (rcvr,xmtr) interactor pair we previously registered.
	//! Called from TurnOff().
	void DeregisterInterferenceInteractors();
	...
	//! Light-weight read-only data exposed to sensors that do not hold weapon instances.
	struct SensorView
	{
		bool         mFound{ false };
		bool         mJammerInitialized{ false };
		size_t       mModeCount{ 0 };
		size_t       mTargetCount{ 0 };
		size_t       mGroupCount{ 0 };
		TargetingDataMap mTargetData;
		std::string  mWeaponName;
		std::string  mPlatformName;
		
		// The following fields are exposed for external sensors that do not hold a weapon instance.
		// They map to `RadarModel::Jammer` public members (fj/Bj/Lj/EqPower/...).
		std::string  mJammerType{ "" };
		double       mFj{ 0.0 };       // jammer center frequency (Hz)
		double       mBj{ 0.0 };       // jammer bandwidth (Hz)
		double       mLj_dB{ 0.0 };    // jammer internal loss in dB
		double       mEqPower{ 0.0 };  // equivalent jammer power (W)
		double       mKJC{ 1.0 };      // model constant
		double       mKJI{ 1.0 };      // model constant
		int          mFakeTargetNum{ 0 };

		// Geometry/pointing used by the simplified radar model.
		// Coordinates follow the same convention as TricorderSensor.cpp (LLA used as x/y/z inputs).
		double       mJammerLon{ 0.0 };
		double       mJammerLat{ 0.0 };
		double       mJammerAlt{ 0.0 };
		double       mBeamDirectionAz{ 180 }; // degrees
		double       mBeamDirectionEl{ 90 }; // degrees
		double       mBeamMinScanAz{ 0}; // degrees
		double       mBeamMaxScanAz{ 360 }; // degrees
		double       mBeamScanEl{ 90 }; // degrees

		double       AntennaGain{ 0 };
		double		 LJ{ 0 };
		WsfEM_XmtrRcvr antenna;
		
		bool isturnOn;
		double yaw;
	};

	//! Build a read-only snapshot from this weapon instance.
	SensorView GetSensorViewSnapshot() const;

	//! Find a PhaserWeapon on a platform and return its snapshot.
	//! If aWeaponName is empty, the first PhaserWeapon component is used.
	static bool TryGetSensorView(WsfPlatform  *aPlatform, SensorView& aOut, WsfStringId aWeaponName = WsfStringId());
	...
	//@}
	inline static WsfStringId getJammerAntennaResult(double                   aSimTime,
		WsfEM_Interaction&       TgtInteraction	) {

		WsfStringId tech;
		WsfEM_Rcvr* rcvrPtr = TgtInteraction.GetReceiver();
		...
		if (interactorCount > 0)
		{
			...
			for (size_t xmtrIndex = 0; xmtrIndex < interactorCount; ++xmtrIndex)
			{
				...
				for (WsfEW_Types::TechniqueId EA_TechId : eaTechniquesToCounter)
				{
					...
					tech = techPtr->GetMitigationClassId();
				}

			}
			
		}
		if (tech.GetString().size()<1) {
			tech = "noise_jamming";
		}
		return tech;
	}
	...
	// Must inherit from WsfRF_Jammer::JammerXmtr (not directly from WsfEM_Xmtr)
	// so that stock code paths inside WsfRF_Jammer (e.g. UpdateRepeaterResult,
	// ComputeJammerEffect) that do `dynamic_cast<WsfRF_Jammer::JammerXmtr*>(xmtr)`
	// succeed and return a valid pointer. Otherwise stock code crashes.
	class  JammerXmtr : public WsfRF_Jammer::JammerXmtr
	{
		friend class PhaserWeapon::JammerBeam;
		friend class PhaserWeapon::JammerMode;

	public:
		//! Nested class PhaserWeapon::JammerXmtr. This class is to provide for handling
	...
	class  JammerMode : public WsfWeapon::WsfWeaponMode
	{
	public:
...
public:
	...
	//! Tracks every (receiver, transmitter) pair we have added via
	//! AddInterferenceInteractor so TurnOff can undo it exactly. Pointers are
	//! non-owning; both lifetimes are managed by their respective platforms.
	std::vector<std::pair<WsfEM_Rcvr*, WsfEM_Xmtr*>> mRegisteredInteractors;

	mutable std::recursive_mutex mMutex;
};

WSF_DECLARE_COMPONENT_ROLE_TYPE(PhaserWeapon, cWSF_COMPONENT_RF_JAMMER)

inline PhaserWeapon::SensorView PhaserWeapon::GetSensorViewSnapshot() const
{
	std::lock_guard<std::recursive_mutex> lock(mMutex);
	SensorView out;
	out.mFound = true;	
	out.mJammerInitialized = mJammerInitialized;
	out.mModeCount = mJammerModeList.size();
	out.mGroupCount = mJammerGroup.size();
	
	out.mTargetData = mTargetDataMap;
	out.mTargetCount = out.mTargetData.size();
	out.mWeaponName = GetName();
	out.mJammerType = "noise_jamming";
	if (GetPlatform() != nullptr)
	{
		out.mPlatformName = GetPlatform()->GetName();
		// Use LLA components as x/y/z inputs to match the convention in TricorderSensor.cpp.
		auto lla = GetPlatform()->GetLocationLLA();
		out.mJammerLon = lla.mLon;
		out.mJammerLat = lla.mLat;
		out.mJammerAlt = lla.mAlt;
	}
	// Populate jammer RF parameters from the first initialized mode (best-effort).
	// These are defaults for the simplified radar jamming model.
	if (!mJammerModeList.empty() && mJammerModeList[0] != nullptr && mJammerModeList[0]->mXmtr)
	{
		 WsfEM_Xmtr xmtr = *mJammerModeList[0]->mXmtr;
		out.isturnOn= mJammerModeList[0]->GetWeapon()->IsTurnedOn();
		out.mFj = xmtr.GetFrequency();
		out.mBj = xmtr.GetBandwidth();
		out.mEqPower = xmtr.GetPower();
		out.LJ=xmtr.GetInternalLoss();
		out.mBeamDirectionAz=xmtr.GetAzimuthBeamwidth();
		out.mBeamDirectionEl = xmtr.GetElevationBeamwidth();
		out.mBeamMinScanAz = mBeamMinScanAz;
		
		out.mBeamMaxScanAz = mBeamMaxScanAz;
		const double internalLossRatio = xmtr.GetInternalLoss(); // absolute ratio, NOT dB
		const double safeLossRatio = std::max(internalLossRatio, 1.0e-12);
		

		out.AntennaGain=xmtr.GetPeakAntennaGain();
		
		out.antenna=xmtr.GetAntenna();
		
		out.yaw=mJammerModeList[0]->GetWeapon()->GetYaw();
		
	}

	// Guard against zero values that would break the simplified jammer math.
	if (out.mFj <= 0.0) { out.mFj = 1.0e9; }
	if (out.mBj <= 0.0) { out.mBj = 1.0e6; }
	if (out.mEqPower <= 0.0) { out.mEqPower = 1.0; }
	if (std::isnan(out.mLj_dB) || std::isinf(out.mLj_dB)) { out.mLj_dB = 0.0; }
	
	out.mFakeTargetNum = 1;
	// TODO: If you later model jammer pointing, fill mBeamDirectionAz/mBeamDirectionEl using antenna pointing.
	return out;
}

inline bool PhaserWeapon::TryGetSensorView(WsfPlatform  *aPlatform,SensorView& aOut, WsfStringId aWeaponName)
{
	const PhaserWeapon* weaponPtr = nullptr;
	if (!aWeaponName.IsNull())
	{
		weaponPtr = aPlatform->GetComponent<PhaserWeapon>(aWeaponName);
	}
	if (weaponPtr == nullptr)
	{
		weaponPtr = aPlatform->GetComponent<PhaserWeapon>();
	}
	if (weaponPtr == nullptr|| !weaponPtr->IsA_TypeOf("PHASER_WEAPON"))
	{
		return false;
	}
	aOut = weaponPtr->GetSensorViewSnapshot();
	return true;
}
