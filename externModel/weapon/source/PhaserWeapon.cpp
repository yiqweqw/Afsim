...
PhaserWeapon::PhaserWeapon(const WsfScenario& aScenario)
	: WsfRF_Jammer(aScenario)
	, mJammerGroup()
...
// virtual
bool PhaserWeapon::Initialize(double aSimTime)
{
...
//virtual
const int* PhaserWeapon::GetComponentRoles() const
{
...
// virtual
void* PhaserWeapon::QueryInterface(int aRole)
{
...
// virtual
bool PhaserWeapon::ProcessMessage(double aSimTime, const WsfMessage& aMessage)
{
...
// virtual
void PhaserWeapon::TurnOff(double aSimTime)
{
	if (mJammerInitialized && (GetEM_XmtrCount(mModeListPtr->GetCurrentMode()) > 0))
	{
		StopJamming(aSimTime, 0.0, 0.0, 0, WsfTrackId());
	}
	// De-register all of our jammer transmitters from every receiver they were
	// added to. Must run BEFORE WsfWeapon::TurnOff so the xmtr pointers are still valid.
	DeregisterInterferenceInteractors();
	WsfWeapon::TurnOff(aSimTime);
}

// virtual
void PhaserWeapon::TurnOn(double aSimTime)
{
	...
	if ((GetEM_XmtrCount(mModeListPtr->GetCurrentMode()) == 0) && mJammerInitialized)
	{
		...
		mBeamMinScanAz=GetEM_Xmtr(0).GetAntenna()->GetMinimumAzimuthScanLimit();
		mBeamMaxScanAz = GetEM_Xmtr(0).GetAntenna()->GetMaximumAzimuthScanLimit();
	}

	// Register our jammer transmitters as interference interactors on every
	// co-band RF receiver in the simulation. This is what allows stock
	// WsfRadarSensor to discover us via rcvr->GetInterferenceInteractorEntry(i)
	// inside WsfRF_Jammer::ComputeTotalJammerEffects.
	RegisterInterferenceInteractors();
}

// virtual
size_t PhaserWeapon::GetEM_XmtrCount() const
{
	size_t xmtrCount = 0;
...
// virtual
WsfEM_Xmtr& PhaserWeapon::GetEM_Xmtr(size_t aIndex)
{
	JammerMode* modePtr = mJammerModeList[mModeListPtr->GetCurrentMode()];
...
// virtual
size_t PhaserWeapon::GetEM_XmtrCount(size_t aModeIndex) const
{
	size_t xmtrCount = 0;
...
// virtual
WsfEM_Xmtr& PhaserWeapon::GetEM_Xmtr(size_t aModeIndex, size_t aIndex)
{
...
// virtual
WsfEM_Xmtr& PhaserWeapon::GetEM_Xmtr(size_t aModeIndex, size_t aBeamIndex, size_t aSpotIndex)
{
	if (aModeIndex < mJammerModeList.size())
	{
		JammerMode* modePtr = mJammerModeList[aModeIndex];
...
// virtual
WsfEM_Xmtr& PhaserWeapon::GetMasterXmtr()
{
	JammerMode* modePtr = mJammerModeList[mModeListPtr->GetCurrentMode()];
...
WsfEM_Xmtr&
PhaserWeapon::GetJammerXmtr(double aSimTime, double aFrequency, double aBandwidth, size_t aBeamNumber, WsfTrackId aTrackId)
{
...
double PhaserWeapon::GetMinimumFrequency() const
{
	// Get a pointer to the current mode
	JammerMode* modePtr = mJammerModeList[mModeListPtr->GetCurrentMode()];
...
double PhaserWeapon::GetMaximumFrequency() const
{
	// Get a pointer to the current mode
	JammerMode* modePtr = mJammerModeList[mModeListPtr->GetCurrentMode()];
	return (modePtr->mXmtr->GetFrequency() + 0.5 * modePtr->mXmtr->GetBandwidth());
...
// virtual
unsigned int PhaserWeapon::GetActiveRequestCount() const
{
...
// virtual
unsigned int PhaserWeapon::GetMaximumRequestCount() const
{
...
// virtual
size_t PhaserWeapon::GetActiveBeamCount() const
{
	size_t beamCount = 0;
	// Need this check because DIS platform constructor is trying to get xmtr count before initialization
	if (mJammerInitialized)
...
// virtual
size_t PhaserWeapon::GetMaximumBeamCount() const
{
	size_t beamCount = 0;
	// Need this check because DIS platform constructor is trying to get xmtr count before initialization
	if (mJammerInitialized)
...
// virtual
size_t PhaserWeapon::GetActiveSpotCount(size_t aBeamNumber) const
{
	size_t spotCount = 0;
	// Need this check because DIS platform constructor is trying to get xmtr count before initialization
	if (mJammerInitialized)
...
// virtual
size_t PhaserWeapon::GetMaximumSpotCount(size_t aBeamNumber) const
{
	size_t spotCount = 0;
	// Need this check because DIS platform constructor is trying to get xmtr count before initialization
	if (mJammerInitialized)
...
// virtual
double PhaserWeapon::GetQuantityRemaining() const
{
	return (GetMaximumRequestCount() - GetActiveRequestCount());
}

// virtual
bool PhaserWeapon::StartJammingTrack(double aSimTime, WsfStringId aTechniqueId, const WsfTrack* aTrackPtr)
{
...
// virtual
bool PhaserWeapon::StartJamming(double      aSimTime,
	double      aFrequency,
	double      aBandwidth,
	size_t      aBeamNumber,
...
// virtual
bool PhaserWeapon::StopJamming(double aSimTime, double aFrequency, double aBandwidth, size_t aBeamNumber, WsfTrackId aTrackId)
{
	// Set the default return value
...
// virtual
bool PhaserWeapon::UpdateJamming(double      aSimTime,
	size_t      aXmtrIndex,
	double      aFrequency,
	double      aBandwidth,
	WsfStringId aTechniqueId,
...
// virtual
void PhaserWeapon::SetTargetIndex(int aId, size_t aTargetIndex)
{
	mTargetDataMap[aId].mTargetIndex = aTargetIndex;
}

// virtual
size_t PhaserWeapon::GetTargetIndex(int aId) const
{
...
void PhaserWeapon::SetTargetTrackId(int aId, const WsfTrackId& aTargetTrackId)
{
...
// virtual
WsfTrackId PhaserWeapon::GetTargetTrackId(int aId) const
{
	WsfTrackId trackId(nullptr, 0);
	auto       tdmi = mTargetDataMap.find(aId);
	if (tdmi != mTargetDataMap.end())
...
void PhaserWeapon::SetTargetedLocationWCS(int aId, const double aTargetedLocationWCS[3])
{
	UtVec3d::Set(mTargetDataMap[aId].mTargetedLocationWCS, aTargetedLocationWCS);
	mTargetDataMap[aId].mTargetedLocationValid = true;
}

void PhaserWeapon::GetTargetedLocationWCS(int aId, double aTargetedLocationWCS[3]) const
{
	auto tdmi = mTargetDataMap.find(aId);
	if (tdmi != mTargetDataMap.end())
	{
...
// virtual
bool PhaserWeapon::TargetedLocationValid(int aId) const
{
	bool locValid = false;
	auto tdmi = mTargetDataMap.find(aId);
	if (tdmi != mTargetDataMap.end())
...
bool PhaserWeapon::SelectTarget(double aSimTime, WsfEM_Xmtr* aXmtr)
{
...
bool PhaserWeapon::ClearTarget(WsfEM_Xmtr* aXmtr)
{
	if (static_cast<PhaserWeapon*>(aXmtr->GetArticulatedPart())->TargetedLocationValid(aXmtr->GetUniqueId()))
	{
...
WsfLocalTrack* PhaserWeapon::FindTargetTrack(const WsfTrackId& aTargetTrackId) const
{
...
// static
bool PhaserWeapon::UpdateRepeaterResult(double                   aSimTime,
	WsfEM_Xmtr*              aXmtr,
	...
	const WsfEM_Interaction& aTargetInteraction)
{
	...
	{
		jammerModePtr->mUseRepeater = true;
...
// virtual
bool PhaserWeapon::SelectEA_Technique(double      aSimTime,
	double      aFrequency,
	double      aBandwidth,
	size_t      aBeamNumber,
	WsfStringId aTechniqueId,
...
// virtual
bool PhaserWeapon::DeselectEA_Technique(double      aSimTime,
	double      aFrequency,
	double      aBandwidth,
	size_t      aBeamNumber,
	WsfStringId aTechniqueId,
...
// virtual
bool PhaserWeapon::SetEA_DeltaGainTechnique(double      aSimTime,
	WsfStringId aTechniqueId,
	bool        aOnOff,
...
// virtual
bool PhaserWeapon::WithinFieldOfView(double aSimTime, WsfPlatform* aPlatformPtr)
{
...
// virtual
bool PhaserWeapon::WithinFieldOfView(double aSimTime, WsfTrack* aTrackPtr)
{
	bool withinFieldOfView(false);
...
// static
double PhaserWeapon::ComputeJammerPower(double                   aSimTime,
...
// static
void PhaserWeapon::ComputeJammerPowers(double                   aSimTime,
	WsfEM_Xmtr*              aJammerXmtrPtr,
	const WsfEM_Interaction& aTargetInteraction,
	WsfEM_Interaction&       aJammerInteraction,
...
// static
void PhaserWeapon::ComputeSingleJammerEffect(double             aSimTime,
	WsfEM_Xmtr*        aJammerXmtrPtr,
...
// static
void PhaserWeapon::ComputeTotalJammerEffects(double aSimTime, WsfEM_Interaction& aTargetInteraction, bool aApplyEW_Effects)
{
	WsfEW_Result* tgtInteractionPtr = WsfEW_Result::Find(aTargetInteraction);
	if (tgtInteractionPtr == nullptr)
	{
...
void PhaserWeapon::ComputeJammerEffect(double                   aSimTime,
	WsfEM_Xmtr*              aXmtrPtr,
	const WsfEM_Interaction& aTargetInteraction,
	...
	WsfEW_Effect*            aProcessingEW_EffectPtr /*= 0*/)
{
	...
	PhaserWeapon* jammerPtr = static_cast<PhaserWeapon*>(aXmtrPtr->GetArticulatedPart());
...
// static
double PhaserWeapon::ComputeReceiverGains(WsfEM_Rcvr* aRcvrPtr)
{
...
// private
void PhaserWeapon::InitializeJammerGroup()
{
	...
	if (mJammerGroupNameId != 0)
	{
		...
		for (WsfComponentList::RoleIterator<WsfWeapon> iter(*GetPlatform()); !iter.AtEnd(); ++iter)
		{
			...
			if (weaponPtr != this)
			{
				PhaserWeapon* jammerPtr = dynamic_cast<PhaserWeapon*>(weaponPtr);
...
unsigned int PhaserWeapon::GetActiveGroupCount() const
{
...
void PhaserWeapon::SensorTrackDropped(double aSimTime, WsfTrackId aTrackId, size_t aTargetIndex)
{
	JammerMode* modePtr = mJammerModeList[mModeListPtr->GetCurrentMode()];
	if (modePtr->mUseRepeater)
	{
...
void PhaserWeapon::SensorTrackInitiated(double aSimTime, const WsfTrack* aTrackPtr)
{
	JammerMode* modePtr = mJammerModeList[mModeListPtr->GetCurrentMode()];
	if (modePtr->mUseRepeater)
	{
...
void PhaserWeapon::SensorTrackUpdated(double aSimTime, const WsfTrack* aTrackPtr)
{
	JammerMode* modePtr = mJammerModeList[mModeListPtr->GetCurrentMode()];
	if (modePtr->mUseRepeater)
	{
...
PhaserWeapon::JammerXmtr::JammerXmtr(WsfEM_Antenna* aAntennaPtr)
	: WsfRF_Jammer::JammerXmtr(aAntennaPtr)
	, mIgnoredCategories()
	, mIgnoredSides()
	, mIgnoredDomains(0)
...
PhaserWeapon::JammerXmtr::JammerXmtr(const JammerXmtr& aSrc, WsfEM_Antenna* aAntennaPtr)
	: WsfRF_Jammer::JammerXmtr(aSrc, aAntennaPtr)
...
// virtual
bool PhaserWeapon::JammerXmtr::Initialize(WsfSimulation& aSimulation)
{
	if (mIgnoreSameSide &&
...
// virtual
bool PhaserWeapon::JammerXmtr::ProcessInput(UtInput& aInput)
{
	bool        myCommand = true;
	std::string command(aInput.GetCommand());
	if (command == "ignore")
...
// virtual
bool PhaserWeapon::JammerXmtr::AllowInteractionWith(WsfEM_Rcvr* aRcvrPtr)
{
...
// public
void PhaserWeapon::JammerXmtr::GetFrequencyBand(double aFrequencyBand[2]) const
{
	UtVec2d::Set(aFrequencyBand, mFrequencyBand);
}

PhaserWeapon::JammerMode::JammerMode(const WsfScenario& aScenario)
...
PhaserWeapon::JammerMode::JammerMode(const JammerMode& aSrc)
	: WsfWeaponMode(aSrc)
...
// virtual
PhaserWeapon::JammerMode::~JammerMode()
{
...
// virtual
WsfMode* PhaserWeapon::JammerMode::Clone() const
{
	return new JammerMode(*this);
}

// virtual
bool PhaserWeapon::JammerMode::Initialize(double aSimTime, WsfWeapon* aWeaponPtr)
{
	// Set the default return value
	bool ok = true;
...
// virtual
bool PhaserWeapon::JammerMode::ProcessInput(UtInput& aInput)
{
	bool        myCommand = true;
	std::string command(aInput.GetCommand());
...
// virtual
void PhaserWeapon::JammerMode::Deselect(double aSimTime)
{
...
// virtual
void PhaserWeapon::JammerMode::Select(double aSimTime)
{
...
void PhaserWeapon::JammerMode::AdjustPowerDistribution(PowerDistribution aGroupPowerDistribution,
...
size_t PhaserWeapon::JammerMode::GetActiveBeamCount() const
{
	return static_cast<unsigned int>(mActiveBeamPtrs.size());
}

size_t PhaserWeapon::JammerMode::GetAvailableBeamCount() const
{
	return static_cast<unsigned int>(mAvailableBeamPtrs.size());
}

PhaserWeapon::JammerBeam* PhaserWeapon::JammerMode::GetJammerBeam(size_t aBeamNumber, bool& aIsActive) const
{
...
bool PhaserWeapon::JammerMode::GetActiveBeamSpotIndexes(double            aFrequency,
	double            aBandwidth,
	size_t            aBeamNumber,
	WsfTrackId        aTrackId,
...
PhaserWeapon::JammerBeam* PhaserWeapon::JammerMode::GetNextAssignableBeam(size_t aBeamNumber, bool& aIsActive) const
{
	JammerBeam* beamPtr(nullptr);
	aIsActive = false;
...
bool PhaserWeapon::JammerMode::AdjustFrequencyBandwidth(double& aFrequency, double& aBandwidth)
{
	bool adjusted = false;
...
PhaserWeapon::JammerBeam::JammerBeam(unsigned int aBeamNumber)
...
PhaserWeapon::JammerBeam::JammerBeam(const JammerBeam& aSrc, unsigned int aBeamNumber)
	: mBeamNumber(aBeamNumber)
	, mActiveSpotPtrs()
	, mSpotTrackIds()
...
// virtual
PhaserWeapon::JammerBeam::~JammerBeam()
{
...
// virtual
bool PhaserWeapon::JammerBeam::Initialize(WsfSimulation& aSimulation, JammerXmtr& aXmtr)
{
...
// virtual
bool PhaserWeapon::JammerBeam::ProcessInput(UtInput& aInput)
{
	bool        myCommand = true;
	std::string command(aInput.GetCommand());
...
// virtual
void PhaserWeapon::JammerBeam::Deselect(double aSimTime)
{
...
// virtual
void PhaserWeapon::JammerBeam::Select(double aSimTime)
{
...
unsigned int PhaserWeapon::JammerBeam::GetActiveSpotCount() const
{
	return static_cast<unsigned int>(mActiveSpotPtrs.size());
}

unsigned int PhaserWeapon::JammerBeam::GetAvailableSpotCount() const
{
	return static_cast<unsigned int>(mAvailableSpotPtrs.size());
}

bool PhaserWeapon::JammerBeam::GetActiveSpotIndex(double        aFrequency,
	double        aBandwidth,
	WsfTrackId    aTrackId,
...
void PhaserWeapon::JammerBeam::SetBeamNumber(unsigned int aBeamNumber) { mBeamNumber = aBeamNumber; }
unsigned int PhaserWeapon::JammerBeam::GetBeamNumber() const { return mBeamNumber; }
size_t PhaserWeapon::JammerMode::GetActiveXmtrSpotCount() const { return mActiveXmtrSpotPtrs.size(); }
unsigned int PhaserWeapon::JammerMode::GetMaximumXmtrSpotCount() const { return mMaximumNumSpots; }
const PhaserWeapon::TargetingDataMap& PhaserWeapon::GetTargetDataMap() const { return mTargetDataMap; }

// =================================================================================================
//! Walk every RF receiver in the simulation and register each of our jammer
//! transmitters as an interference interactor. This is what makes stock
//! WsfRadarSensor able to see us as a jammer: at detection time
//! WsfRF_Jammer::ComputeTotalJammerEffects iterates the receiver's interactor
//! list, which we populate here.
void PhaserWeapon::RegisterInterferenceInteractors()
{
	WsfSimulation* simPtr = GetSimulation();
	if (simPtr == nullptr) { return; }

	const size_t myXmtrCount = GetEM_XmtrCount();
	if (myXmtrCount == 0) { return; }

	const size_t platformCount = simPtr->GetPlatformCount();
	for (size_t pi = 0; pi < platformCount; ++pi)
	{
		WsfPlatform* platformPtr = simPtr->GetPlatformEntry(pi);
		if ((platformPtr == nullptr) || (platformPtr == GetPlatform())) { continue; }

		for (WsfComponentList::RoleIterator<WsfSensor> it(*platformPtr); !it.AtEnd(); ++it)
		{
			WsfSensor* sensorPtr = *it;
			const size_t rcvrCount = sensorPtr->GetEM_RcvrCount();
			for (size_t ri = 0; ri < rcvrCount; ++ri)
			{
				WsfEM_Rcvr& rcvr = sensorPtr->GetEM_Rcvr(ri);
				for (size_t xi = 0; xi < myXmtrCount; ++xi)
				{
					WsfEM_Xmtr& xmtr = GetEM_Xmtr(xi);
					if (rcvr.CanInteractWith(&xmtr))
					{
						rcvr.AddInteractor(&xmtr);  // routes to mInterferenceInteractors because cXF_INTERFERER
						mRegisteredInteractors.emplace_back(&rcvr, &xmtr);
					}
				}
			}
		}
...
// =================================================================================================
//! Remove every (rcvr,xmtr) interactor pair we previously registered.
void PhaserWeapon::DeregisterInterferenceInteractors()
{
	for (auto& pair : mRegisteredInteractors)
	{
		if ((pair.first != nullptr) && (pair.second != nullptr))
		{
			pair.first->RemoveInteractor(pair.second);
		}
	}
	mRegisteredInteractors.clear();
}
std::recursive_mutex& PhaserWeapon::GetMutex() { return mMutex; }